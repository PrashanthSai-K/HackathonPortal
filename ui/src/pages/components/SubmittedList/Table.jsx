...
export default function Table() {
    ...
    const tableLoader = () => (
        <div className='flex items-center justify-center min-h-96 h-full w-full '>
            <p>No submittedList Available</p>
        </div>
    )
    ...
    const [submittedListCall, isLoading] = useActionState(fetchSubmittedList, true);
    ...
    const selectTeam = async (data) => {
        try {
            ...
            const response = await adminPostRequest("/submittedList/select", { ps_id: data.ps_id, team_id: data.team_id });
    ...
    const unselectTeam = async (data) => {
        try {
            ...
            const response = await adminPostRequest("/submittedList/unselect", {
                ps_id: data.ps_id,
        ...
        <>
            {isLoading ?
                ...
                <div className="card w-full pt-3 flex items-center justify-center px-4 md:px-8 ">
                    <DataTable sortIcon={customSortIcon} value={submittedList} emptyMessage={tableLoader} paginator={submittedList.length > 5} rows={5} rowsPerPageOptions={[5, 10, 25, 50]} globalFilter={globalFilter} paginatorClassName='text-black' stripedRows className='border rounded-lg overflow-hidden w-11/12 min-h-96 max-w-screen-lg'>
                        ...
                        <Column field="stage" header="Status" body={statusTemplate} align={"center"} bodyStyle={{ height: "5rem" }} headerClassName='border-b text-end font-medium bg-violet-900 text-sm  relative z-10' className='border-b-2 border-r-2 p-1 text-sm'></Column>
