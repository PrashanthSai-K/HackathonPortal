    ...
    return (
        <>
            ...
            <Dialog header="Problem Statement Details" headerClassName="p-2 w-full" visible={visible} onHide={() => { if (!visible) return; setVisible(false); }}
                className='p-3 md:p-3 bg-white flex items-center justify-center' style={{maxWidth:"80%"}} breakpoints={{ '960px': '75vw', '641px': '90vw' }}>
                <center className='pb-4 w-full'>
                    <DataTable value={modalData} stripedRows className='border rounded-lg overflow-hidden w-full max-h-full'>
